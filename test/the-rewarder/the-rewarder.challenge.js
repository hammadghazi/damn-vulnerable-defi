...
describe("[Challenge] The rewarder", function () {
  ...
  it("Exploit", async function () {
    ...
    // Calling 'executeFlashloan' function after 5 days so that reward pool can give deposit rewards
    // If you're the first depositor (when pool is eligible to give new round rewards)
    // your deposit will be considered as if you deposited it in previous round
    // as reward pool first mints 'accountToken' to the depositor address and then
    // records a snapshot (if new round reward is started), after updating snapshot id
    // it checks total deposit and userBalance at last snapshot id (which makes pool think that you
    // deposited it in previous round - though you deposited it in current round, so in in ideal work
    // you should get the reward in next round) and then it calculates/ distributes the reward

    // Bug can be fixed if we swap 'accToken.mint' with 'distributeRewards' in deposit function of 'RewardPool' contract
    await flashloanReceiver
      .connect(attacker)
      .executeFlashloan(
        this.flashLoanPool.address,
